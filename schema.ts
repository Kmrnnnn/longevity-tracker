...
export const subscriptionPlans = mysqlTable("subscription_plans", {
  ...
  priceInCents: int("priceInCents").notNull(), // 价格（分）
...
// 更新users表添加Stripe字段
export const usersWithStripe = mysqlTable("users_stripe", {
  id: int("id").autoincrement().primaryKey(),
  stripeCustomerId: varchar("stripeCustomerId", { length: 255 }).unique(),
  subscriptionStatus: mysqlEnum("subscriptionStatus", ["free", "active", "canceled", "past_due"]).default("free"),
  subscriptionTier: mysqlEnum("subscriptionTier", ["free", "pro", "premium"]).default("free"),
});

export type UserWithStripe = typeof usersWithStripe.$inferSelect;
export type InsertUserWithStripe = typeof usersWithStripe.$inferInsert;
